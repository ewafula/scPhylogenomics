...
def csp_load_data(data_dir, is_genotype = False, is_gzip = True):
    ...
    base_vcf_path = os.path.join(data_dir, "cellSNP.base.vcf" + vcf_suffix)

    if not os.path.exists(base_vcf_path):
        # Fallback if the user didn't gzip, or vice versa, though input flag controls this.
        # Trying to be robust if file exists without gz
        if os.path.exists(os.path.join(data_dir, "cellSNP.base.vcf")):
            base_vcf_path = os.path.join(data_dir, "cellSNP.base.vcf")
            is_gzip = False
...
def csp_load_vcf(fn):
    ...
    # Fix: Ensure we strip newline chars from the header split
    content.columns = pre_line.strip().split("\t")
    # Remove the # from #CHROM to make it easier to query
...
def csp_save_data(adata, out_dir):
    ...
    # Prepare OBS for VCF saving (Need to rename CHROM back to #CHROM for standard VCF)
...
def csp_save_vcf(df, comment, fn, is_gzip = True):
    df_file = fn + ".df"
    # Ensure column order matches VCF standard if possible, though passed df usually has correct structure
