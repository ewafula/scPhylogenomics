...
5. **[Phylogeny Inference](https://github.com/ewafula/scPhylogenomics/tree/main/analyses/phylogeny-inference):** Reconstructs evolutionary histories by integrating `maximum-likelihood` tree building ([FastTree](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0009490) or [IQ-TREE](https://ecoevorxiv.org/repository/view/8916/)) with advanced clonal population clustering. The workflow offers a unified interface for clonal resolution, allowing users to choose between a `Deep Learning Variational Autoencoder (VAE)` approach ([SNPmanifold](https://pmc.ncbi.nlm.nih.gov/articles/PMC12465888/)) or a `Hybrid Hierarchical Weighted Non-Negative Matrix Factorization (WNMF)` algorithm.
